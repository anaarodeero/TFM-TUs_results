...
#5: Hardrules
# 5.1: Now
bicleaner-hardrules 4_$file.$l1-$l2.fixed 5_$file.$l1-$l2.cleaned --annotated_output -s $l1 -t $l2 --scol 1 --tcol 2 -c config.yaml
# 5.2: Hardrules used in the pre-trained model
#python ./bicleaner-v0.14/bicleaner/bicleaner_hardrules.py 4_$file.en-es.fixed 5_$file.en-es.cleaned -s en -t es --scol 1 --tcol 2 --annotated_output --disable_lm_filter --disable_porn_removal
